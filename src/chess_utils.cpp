...
void ChessUtils::printBoard(const char board[8][8]) {
  ...
  for (int row = 0; row < 8; row++) {
    ...
    rowStr += " " + String(8 - row);
    webLog.println(rowStr);
  }
  webLog.println("  a b c d e f g h");
  webLog.println("===================");
}
...
bool ChessUtils::ensureNvsInitialized() {
  ...
  if (err != ESP_OK) {
    webLog.printf("[NVS] Init failed with error 0x%x, erasing and retrying...\n", err);
    nvs_flash_erase();
    err = nvs_flash_init();
    if (err == ESP_OK) {
      webLog.println("[NVS] Successfully initialized after erase");
    } else {
      webLog.printf("[NVS] Still failed after erase: 0x%x\n", err);
    }
  }
  return err == ESP_OK;
}
